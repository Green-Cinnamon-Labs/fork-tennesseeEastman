...
Com IDV(1) ativo desde t=0 e os 3 controladores P operando, o sistema deve:
- Atingir um novo ponto de operação perturbado dentro de ~5–10h
- Manter pressão, sep level e stripper level dentro dos limites ISD durante a transição
- Mostrar desvio visível em temperatura do reator, recycle flow e composição relativa (UCVR components) em relação ao baseline do Exp 10

Se o sistema sobreviver 20h sem ISD, os 3 controladores são suficientes para rejeitar IDV(1).
