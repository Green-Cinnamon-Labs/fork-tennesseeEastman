...
## Experimento 11 — Rejeição de distúrbio IDV(1): variação na razão A/C do feed

### Observação

Com o baseline validado (Exp 10), o sistema está pronto para os testes de rejeição de distúrbio. O primeiro distúrbio canônico do TEP é o **IDV(1)**: step na composição da alimentação combinada de A e C (stream 4) — especificamente, mudança na razão A/C enquanto a composição de B permanece constante. Esse distúrbio afeta diretamente a estequiometria da reação e é o mais amplamente estudado na literatura TEP.

**Condição inicial:** `te_exp3_snapshot.toml` (idêntica ao Exp 10 — garantindo comparabilidade direta)

### Hipótese

A análise do Exp 11 será estruturada em **três níveis distintos**, da condição necessária à condição suficiente:

**Nível 1 — Segurança operacional** (condição necessária, não suficiente):
Pressão, nível do reator, sep level e stripper level permanecem dentro dos limites ISD por 20h. Se falhar aqui, os 3 controladores não garantem operação segura sob IDV(1).

**Nível 2 — Qualidade de rejeição do distúrbio** (variáveis de processo):
Comparação quantitativa com o Exp 10 em: pressão, temperatura do reator, recycle flow, purge valve. Métricas relevantes: (a) magnitude do desvio máximo em relação ao baseline; (b) tempo de acomodação (quanto tempo até as variáveis voltarem a trajetória paralela ao baseline); (c) novo patamar estacionário (offset permanente, se houver). Sobreviver ao ISD com um offset de 50 kPa de pressão ou 20°C de temperatura é qualitativamente diferente de um desvio de 5 kPa.

**Nível 3 — Efeito na composição e inventário** (variáveis internas):
Comparação de UCVR total e componentes A–H (Exp 11 vs Exp 10). O IDV(1) muda a razão A/C — espera-se que afete a relação G/H no inventário e possivelmente o ritmo de acúmulo. Se o inventário crescer mais rápido ou lento que o baseline, isso indica que o IDV(1) desloca o balanço de massa.
...
Condição inicial, dt, controladores e demais parâmetros: **idênticos ao Exp 10**.

### Resultado

**ISD disparado em t ≈ 2.04h** — gatilho primário: **Reactor P high (>3000 kPa)** (confirmado no dashboard; temperatura e níveis ainda dentro dos limites no instante do shutdown).

Análise por nível:

**Nível 1 — Segurança operacional:** ❌ FALHOU. Pressão subiu de ~2680 → 3000 kPa em ~2h.

**Nível 2 — Qualidade de rejeição (variáveis de processo):**
- **Pressão**: subida contínua sem estabilização; purge valve chegou a ~70% (~30 pp acima do nominal) sem conseguir compensar a geração de gás
- **Temperatura**: permaneceu em ~120°C até próximo ao ISD — não foi o gatilho primário
- **Reactor Level**: subiu ~70% → ~85% (efeito secundário VLE do aumento de pressão, confirmando o mecanismo já identificado no Exp 9)
- **Recycle Flow**: caiu de ~26.8 → ~25.5 kscmh — a mudança de composição do loop reduziu o fluxo de reciclo
- **Purge**: abriu progressivamente de ~39% → ~70%, confirmando que o controlador P respondeu corretamente mas não teve ganho/velocidade suficientes

**Nível 3 — Composição/inventário:**
- Σ UCVR: explodiu de ~350 → ~410 kmol em 2h (+30 kmol/h vs +0.35 kmol/h no baseline — 85× mais rápido)
- Σ UCVV: de ~333 → ~370 kmol em 2h (+18 kmol/h)
- UCVR components: G e H cresceram dominantemente, mantendo composição relativa constante — o IDV(1) acelerou a geração proporcional de vapor, não mudou seletividade

### Conclusão

**Nível 1 reprovado: os 3 controladores P atuais não garantem operação segura sob IDV(1).**

O mecanismo de falha é identificado: o IDV(1) (mudança na razão A/C) aumenta a taxa de reação/geração de vapor, acumulando gás mais rápido do que o purge consegue remover. O controlador de pressão com Kp=0.1 chegou a 70% de abertura da purge — respondeu corretamente — mas a taxa de acúmulo (~30 kmol/h vs capacidade de remoção do purge) excedeu a capacidade física do controlador lento.

**A causa não é ausência de resposta, mas lentidão de resposta.** Com Kp=0.1, a purge valve aumenta apenas 10 pp para cada 100 kPa de desvio de pressão. Para conter o acúmulo de IDV(1), o controlador precisaria abrir a purge muito mais rapidamente nos primeiros minutos, antes que o inventário escape ao controle.

**Próximo passo:** aumentar o ganho do controlador de pressão (Kp: 0.1 → 1.0) para ver se uma resposta 10× mais agressiva da purge consegue conter o IDV(1).

---

## Experimento 12 — IDV(1) com controlador de pressão mais agressivo (Kp=1.0)

### Observação

No Exp 11, o controlador de pressão (Kp=0.1) chegou a ~70% de abertura da purge mas não impediu o ISD por pressão em t=2h. A purge valve abriu de forma gradual e insuficiente para conter a taxa de acúmulo de gás induzida pelo IDV(1). O controlador respondeu na direção correta mas com ganho 10× baixo.

### Hipótese

Aumentando Kp de 0.1 para 1.0 (10×), o controlador de pressão deve abrir a purge valve muito mais rapidamente em resposta ao aumento de pressão. Com Kp=1.0, a purge atinge 100% quando a pressão estiver apenas ~60 kPa acima do setpoint (2705+60=2765 kPa), vs ~300 kPa com Kp=0.1. Se o IDV(1) gera gás a uma taxa constante, essa resposta mais rápida deve ser suficiente para equilibrar o balanço de massa e manter a pressão abaixo de 3000 kPa.

Risco: ganho alto pode causar oscilações de pressão (overshoot/undershoot da purge) ou remoção excessiva de reagentes do loop, alterando a composição e potencialmente desestabilizando outros modos.
...
```rust
// Kp 0.1 → 1.0 (10×): purge atinge 100% com pressão 60 kPa acima do SP
plant.bus.inputs.mv[5] = (40.06 + 1.00 * (reactor_p - 2705.0)).clamp(0.0, 100.0);
```

`active_idv: vec![1]`, `max_sim_time_h: Some(20.0)`, condição inicial `te_exp3_snapshot.toml` — idênticos ao Exp 11.
